...
func patGet(path string, f func(w http.ResponseWriter, r *http.Request)) (*pat.Pattern, func(w http.ResponseWriter, r *http.Request)) {
	path, handleFunc := newrelic.WrapHandleFunc(app, path, f)
	return pat.Get(path), handleFunc
}

func patPost(path string, f func(w http.ResponseWriter, r *http.Request)) (*pat.Pattern, func(w http.ResponseWriter, r *http.Request)) {
	path, handleFunc := newrelic.WrapHandleFunc(app, path, f)
	return pat.Post(path), handleFunc
}
...
func main() {
	...
	// API
	mux.HandleFunc(patPost("/initialize", postInitialize))
	mux.HandleFunc(patGet("/new_items.json", getNewItems))
	mux.HandleFunc(patGet("/new_items/:root_category_id.json", getNewCategoryItems))
	mux.HandleFunc(patGet("/users/transactions.json", getTransactions))
	mux.HandleFunc(patGet("/users/:user_id.json", getUserItems))
	mux.HandleFunc(patGet("/items/:item_id.json", getItem))
	mux.HandleFunc(patPost("/items/edit", postItemEdit))
	mux.HandleFunc(patPost("/buy", postBuy))
	mux.HandleFunc(patPost("/sell", postSell))
	mux.HandleFunc(patPost("/ship", postShip))
	mux.HandleFunc(patPost("/ship_done", postShipDone))
	mux.HandleFunc(patPost("/complete", postComplete))
	mux.HandleFunc(patGet("/transactions/:transaction_evidence_id.png", getQRCode))
	mux.HandleFunc(patPost("/bump", postBump))
	mux.HandleFunc(patGet("/settings", getSettings))
	mux.HandleFunc(patPost("/login", postLogin))
	mux.HandleFunc(patPost("/register", postRegister))
	mux.HandleFunc(patGet("/reports.json", getReports))
	// Frontend
	mux.HandleFunc(patGet("/", getIndex))
	mux.HandleFunc(patGet("/login", getIndex))
	mux.HandleFunc(patGet("/register", getIndex))
	mux.HandleFunc(patGet("/timeline", getIndex))
	mux.HandleFunc(patGet("/categories/:category_id/items", getIndex))
	mux.HandleFunc(patGet("/sell", getIndex))
	mux.HandleFunc(patGet("/items/:item_id", getIndex))
	mux.HandleFunc(patGet("/items/:item_id/edit", getIndex))
	mux.HandleFunc(patGet("/items/:item_id/buy", getIndex))
	mux.HandleFunc(patGet("/buy/complete", getIndex))
	mux.HandleFunc(patGet("/transactions/:transaction_id", getIndex))
	mux.HandleFunc(patGet("/users/:user_id", getIndex))
	mux.HandleFunc(patGet("/users/setting", getIndex))
	// Assets
...
func getNewItems(w http.ResponseWriter, r *http.Request) {
	...
	// items := []Item{}
	itemSellers := []ItemSeller{}
	if itemID > 0 && createdAt > 0 {
		// paging
		err := dbx.Select(&itemSellers,
			"SELECT `item`.`id` AS `item.id`, `item`.`seller_id` AS `item.seller_id`, `item`.`status` AS `item.status`, `item`.`name` AS `item.name`, `item`.`price` AS `item.price`, `item`.`image_name` AS `item.image_name`, `item`.`category_id` AS `item.category_id`, `item`.`created_at` AS `item.created_at`, `seller`.`id` AS `seller.id`, `seller`.`account_name` AS `seller.account_name`, `seller`.`num_sell_items` AS `seller.num_sell_items` FROM `items` AS `item` JOIN `users` AS `seller` ON `item`.`seller_id` = `seller`.`id` WHERE `item`.`status` IN (?,?) AND (`item`.`created_at` < ?  OR (`item`.`created_at` <= ? AND `item`.`id` < ?)) ORDER BY `item`.`created_at` DESC, `item`.`id` DESC LIMIT ?",
			ItemStatusOnSale,
			ItemStatusSoldOut,
		...
		// 1st page
		err := dbx.Select(&itemSellers,
			"SELECT `item`.`id` AS `item.id`, `item`.`seller_id` AS `item.seller_id`, `item`.`status` AS `item.status`, `item`.`name` AS `item.name`, `item`.`price` AS `item.price`, `item`.`image_name` AS `item.image_name`, `item`.`category_id` AS `item.category_id`, `item`.`created_at` AS `item.created_at`, `seller`.`id` AS `seller.id`, `seller`.`account_name` AS `seller.account_name`, `seller`.`num_sell_items` AS `seller.num_sell_items` FROM `items` AS `item` JOIN `users` AS `seller` ON `item`.`seller_id` = `seller`.`id` WHERE `item`.`status` IN (?,?) ORDER BY `item`.`created_at` DESC, `item`.`id` DESC LIMIT ?",
			ItemStatusOnSale,
			ItemStatusSoldOut,
			ItemsPerPage+1,
		)
	...
	for _, itemSeller := range itemSellers {
		// seller, err := getUserSimpleByID(dbx, itemSeller.Item.SellerID)
		// if err != nil {
		// 	outputErrorMsg(w, http.StatusNotFound, "seller not found")
		// 	return
		// }
		category, err := getCategoryByID(dbx, itemSeller.Item.CategoryID)
		if err != nil {
			outputErrorMsg(w, http.StatusNotFound, "category not found")
			return
		}
		itemSimples = append(itemSimples, ItemSimple{
			ID:       itemSeller.Item.ID,
			SellerID: itemSeller.Item.SellerID,
			// Seller:     &seller,
			Seller:     itemSeller.Seller,
			Status:     itemSeller.Item.Status,
			Name:       itemSeller.Item.Name,
			Price:      itemSeller.Item.Price,
			ImageURL:   getImageURL(itemSeller.Item.ImageName),
			CategoryID: itemSeller.Item.CategoryID,
			Category:   &category,
			CreatedAt:  itemSeller.Item.CreatedAt.Unix(),
		})
	}
